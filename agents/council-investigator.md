...
- **Token Stewardship**: Be efficient with tokens. Use `grep_search` or the `offset`/`limit` parameters in `read_file` to extract only relevant sections of large files. Only provide high-value code snippets to the council.
